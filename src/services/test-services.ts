...
class TestService {
  static async createTest(data: CreateTestData): Promise<any> {
    try {
      const formattedData = {
        ...
        visibility_type: data.visibility_type || "view_before_start",
      };
      ...
      // ✅ Fix #8: getApiErrorMessage sanitizes — 422s surface safely, 500s fall back
      toast.error(
  ...
  static async addQuestion(testId: number, data: QuestionData): Promise<any> {
    try {
      ...
      // Re-throw — caller (test-type-form) handles the toast per-question
      throw error;
    }
  }
  ...
  static async addOptions(
    ...
    try {
      ...
      // Re-throw — caller handles the toast per-question
      throw error;
    }
  }
  ...
  static async uploadFile(testId: number, file: File): Promise<any> {
    try {
      ...
      const response = await requests.post("/employers/tests/upload", formData);
      ...
      // ✅ Fix #8: sanitized — never surface raw upload error internals
      toast.error(
        getApiErrorMessage(error, "Failed to upload file. Please try again."),
      );
      throw error;
    }
  }

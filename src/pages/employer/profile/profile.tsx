...
// import { clearAuthStorage } from "@/hooks/forms/use-auth-form";
...
export default function EmployerProfilePage() {
  ...
  // ✅ Uses requests — token read from cookie by Axios interceptor automatically
  const fetchProfile = async () => {
    try {
      const data = await requests.get("/employers/company");
  ...
  const fetchNotificationSettings = async () => {
    try {
      const data = await requests.get("/employers/notification-settings");
      if (data?.data) setNotificationSettings(data.data);
    } catch {
      // Notification settings are non-critical — fail silently
    }
  };
  ...
  const handleNotificationToggle = async (key: keyof NotificationSettings) => {
    ...
    try {
      await requests.put("/employers/notification-settings", newSettings);
      toast.success("Notification settings updated");
    } catch (err: any) {
  ...
  const handleDeleteAccount = async () => {
    ...
    setIsDeleting(true);
    try {
      await requests.delete("/employers/account");
      toast.success("Account deleted successfully");
      clearAuthStorage();
      navigate("/");
    } catch (err: any) {

...
const useJobForm = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Clear corrupted data silently — no console.error with internal details
      clearFormStorage();
    }
  }, [form, hasRestored]);
    ...
    const debounceTimer = setTimeout(() => {
      try {
        ...
        localStorage.setItem(
          STORAGE_KEY,
          JSON.stringify({ formValues, timestamp: new Date().toISOString() }),
        );
      } catch {
        // Silently fail — localStorage save is non-critical
      }
    }, 500);
  ...
  // Show validation errors as toasts when fields are touched
  useEffect(() => {
    const { errors, touchedFields } = form.formState;

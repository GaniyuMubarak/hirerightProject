...
import { clearAuthStorage, readSessionFromCookie } from "@/lib/auth";
...
const useEmailVerificationForm = () => {
  ...
  const onSubmit = async (data: EmailVerificationSchemaType) => {
    try {
      ...
      // ✅ "Already verified" case — read role from HRsessionInfo, not HRuserInfo.
      // HRuserInfo holds tokens only and must never be read for user display data.
      if (response.message === "Email already verified.") {
        toast.success("Email already verified!");
        ...
        if (role === "employer") {
          ...
          // No session data available — force a clean login
          clearAuthStorage();
      ...
      // ✅ setAuthSession was already called inside AuthServices.verifyEmailOTP
      // so both cookies are already written. We only need to dispatch here
      // to update UserContext for the current session.
      dispatch({
        type: "USER_LOGIN",
        payload: {
          ...response,
      ...
      if (status === 422 || status === 400 || status === 410) {
        // ✅ Safe statuses — backend message is user-facing

...
export default function TestTypeForm({
  ...
  const validateForm = (): string | null => {
    const formData = form.getValues();
    if (!formData.testName.trim()) return "Test name is required";
    if (!formData.duration) return "Duration is required";
    if (!formData.experienceLevel) return "Experience level is required";
    if (!formData.submission_type) return "Submission type is required";
    if (!formData.visibility_type) return "Visibility type is required";
  ...
  const handleProceed = async () => {
    try {
      ...
      const createdTest = await TestService.createTest(testData);
      const testId = createdTest.id;
      if (!testId) throw new Error("Test ID not returned from server");
      ...
      if (uploadedFile && creationMethod !== "manual") {
        try {
          ...
          // ✅ Fix #8: don't surface internal upload error details
          toast.warning("File upload failed, continuing with manual questions");
        }
      }
        ...
        for (let i = 0; i < validQuestions.length; i++) {
          ...
          try {
            ...
            if (
              ...
              if (validOptions.length > 0) {
                try {
                  await TestService.addOptions(
                    testId,
                    questionId,
                    validOptions,
                  );
                } catch {
                  // ✅ Fix #8: safe warning, no internal details
                  toast.warning(
                    `Options failed for question ${i + 1}, but question was saved`,
                  );
                }
            ...
            // ✅ Fix #8: use getApiErrorMessage, never surface raw error internals
            toast.warning(
              `Question ${i + 1} could not be saved — ${getApiErrorMessage(questionError, "please try again")}`,
            );
          }
        }
      }
      ...
      // ✅ Fix #8: getApiErrorMessage handles 422 validation errors safely,
      // and falls back to a generic message for 500s and unknown errors
      toast.error(
        getApiErrorMessage(error, "Failed to create test. Please try again."),
      );
    } finally {
      setIsLoading(false);
      setTimeout(() => setUploadProgress(0), 2000);

...
interface ResumeProps {
  ...
  experiences: Array<{
    id: number;
    // ✅ API returns "company" and "position" — accept both for safety
    company?: string;
    company_name?: string;
    position?: string;
    job_title?: string;
  ...
  certifications: Array<{
    id: number;
    name: string;
    // ✅ API returns "issuing_organization" — accept both for safety
    ...
    <div className="bg-white p-6 rounded-lg shadow space-y-6">
      ...
      <div>
        ...
        {certifications && certifications.length > 0 ? (
          <div className="space-y-4">
            {certifications.map((cert) => {
              // ✅ Normalise field name from API
              const org = cert.issuing_organization || cert.organization || "";

   ...
   public function register(Request $request)
{
    ...
    try{
        // 2. Dispatch synchronously so it sends BEFORE the response is sent to the user
        SendOtpEmailJob::dispatchSync($user, $otp);

        // 3. Log success
        Log::info('FINISH: OTP email sent successfully', ['email' => $user->email]);
    }catch (\Exception $e) {
